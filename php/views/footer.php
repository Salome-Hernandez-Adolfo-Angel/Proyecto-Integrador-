
    <!--Inicio del footer-->
    <footer class="bg-dark text-white pt-5pb-4">
        <div class="container text-center text-md-left">
            <div class="row text-center text-md-left">
                <div class="col-md-3 col-lg-3 col-xl-3 mx-auto mt-3">
                    <h5 class="text-uppercase mb-4 font-weight-bold text-warning">Tienda don gualu</h5>
                    <p style="text-align: justify;">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Maiores
                        vero ullam natus optio aspernatur totam, et nisi quos doloremque eaque ab impedit consequatur
                        minima dicta omnis corrupti in, commodi qui?</p>
                </div>
                <div class="col-md-2 col-lg-2 col-xl-2 mx-auto mt-3">
                    <h5 class="text-uppercase mb-4 font-weight-bold text-warning">Proveedores</h5>
                    <p>
                        <a href="#" class="text-white" style="text-decoration: none;">Barcel</a>
                    </p>
                    <p>
                        <a href="#" class="text-white" style="text-decoration: none;">Sabritas</a>
                    </p>
                    <p>
                        <a href="#" class="text-white" style="text-decoration: none;">Marinela</a>
                    </p>
                    <p>
                        <a href="#" class="text-white" style="text-decoration: none;">LALA</a>
                    </p>
                </div>
                <div class="col-md-3 col-lg-2 col-xl-2 mx-auto mt-3">
                    <h5 class="text-uppercase mb-4 font-weight-fold text-warning">Links adicionales</h5>
                    <p>
                        <a href="#" class="text-white" style="text-decoration: none;">Tu cuenta</a>
                    </p>
                    <p>
                        <a href="#" class="text-white" style="text-decoration: none;">Registrarte</a>
                    </p>
                    <p>
                        <a href="#" class="text-white" style="text-decoration: none;">Terminos y condiciones</a>
                    </p>
                    <p>
                        <a href="#" class="text-white" style="text-decoration: none;">Ayuda</a>
                    </p>
                </div>
                <div class="col-md-4 col-lg-3 col-xl-3 mx-auto mt-3">
                    <h5 class="text-uppercase mb-4 font-weight-fold text-warning">Contactos</h5>
                    <p>
                        <i class="fas fa-home mr-3"> Lerma,Estado de México</i>
                    </p>
                    <p>
                        <i class="fas fa-envelope mr-3"> dsg1712@hotmail.com</i>
                    </p>
                    <p>
                        <i class="fas fa-phone mr-3"> +722 887 3633</i>
                    </p>
                    <p>
                        <i class="fas fa-print mr-3"> +72 21 86 75 55</i>
                    </p>
                </div>
            </div>
            <hr class="mb-4">
            <div class="row align-items-center">
                <div class="col-md-7 col-lg-8">
                    <p>Copyright &COPY; All rights reserved by:
                        <a href="" style="text-decoration: none;">
                            <strong class="text-warning">InRed</strong>
                        </a>
                    </p>
                </div>
                <div class="col-md-5 col-lg-4">
                    <div class="text-center text-md-right">
                        <ul class="list-unstyled list-inline">
                            <li class="list-inline-item"><a href="#" class="btn-floating btn-sm text-white"
                                    style="font-size: 24px;"><i class="fab fa-facebook"></i></a></li>
                            <li class="list-inline-item"><a href="#" class="btn-floating btn-sm text-white"
                                    style="font-size: 24px;"><i class="fab fa-instagram"></i></a></li>
                            <li class="list-inline-item"><a href="#" class="btn-floating btn-sm text-white"
                                    style="font-size: 24px;"><i class="fab fa-whatsapp"></i></a></li>
                            <li class="list-inline-item"><a href="#" class="btn-floating btn-sm text-white"
                                    style="font-size: 24px;"><i class="fab fa-linkedin-in"></i></a></li>

                        </ul>

                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </footer>

    